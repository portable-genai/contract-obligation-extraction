# The `third-party-risk-ddq` contract-register feed (versioned proposal)

`contract-obligation-extraction` produces a structured obligation register per contract. The firm-wide Third-Party / Vendor
Risk system, `third-party-risk-ddq` (`third-party-risk-ddq`), consumes that register for the
contractual terms behind a vendor's inherent/residual risk rating. `third-party-risk-ddq` is not built yet, so this
...
enum values), so the regulatory system of record (`obligations-control-mapping`) and this contractual extractor speak one
...
- `kind` distinguishes a contractual register from `obligations-control-mapping`'s regulatory one.
...
- **The risk-flag classification and the deadline status** are `contract-obligation-extraction`'s own view rows, not part of
  ...
  citations, which is the contract that must stay stable for `third-party-risk-ddq`. When `third-party-risk-ddq` is built, the flag and
...
- **Anything unreviewed.** The register a surface returns is the extractor's proposal admitted by
  the engine; every material register sets `requires_human_review` and routes to `human-review-console`. `third-party-risk-ddq` should
  treat an un-approved register as provisional.

## Consuming it

Until `third-party-risk-ddq` exists, a consumer reads the feed from `POST /v1/register` (the `feed` field of the
