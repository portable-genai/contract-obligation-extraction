...
class AgentCard:
    ...
    provider: str = "contract-obligation-extraction"
...
SKILLS: tuple[AgentSkill, ...] = (
    ...
    AgentSkill(
        id="extract_contract_register",
        name="Contract obligation register",
        description=(
            "Read a contract into a tracked, owner-assigned, deadline-bearing register of "
            "obligations, key clauses, dates and risk flags, each linked to its source clause. "
            "The model proposes; the deterministic engine admits, validates flags against the "
            "taxonomy and computes deadline status; a material register routes to human review."
        ),
    ),
    AgentSkill(
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://contract-obligation-extraction.{settings.region}.internal.example/a2a"
